...
class GMT(object):
    ...
    def run_step(self, iter_parameters, iter_limit=1000, debug=False):
        ...
        while True:
            ...
            wavefront(self.dev_Gindicator, self.dev_open, self.dev_cost, self.dev_threshold, self.dev_n, block=(threadsPerBlock,1,1), grid=(nBlocksPerGrid,1))
            ...
            compact(dev_G, dev_Gscan, self.dev_Gindicator, self.dev_waypoints, self.dev_n, block=(threadsPerBlock,1,1), grid=(nBlocksPerGrid,1))     
...
class GMTstream(object):
    ...
    def run_step(self, iter_parameters, iter_limit=1000, debug=False):
        ...
        while True:
            ...
            ########## create Wave front ###############
            nBlocksPerGrid = int(((self.n + threadsPerBlock - 1) / threadsPerBlock))
            wavefront(self.dev_Gindicator, self.dev_open, self.dev_cost, self.dev_threshold, self.dev_n, block=(threadsPerBlock,1,1), grid=(nBlocksPerGrid,1), stream=self.stream1)
            self.dev_threshold += 2*self.dev_radius
            goal_reached = self.dev_Gindicator[self.goal].get() == 1
            ...
            dev_G = cuda.zeros(gSize, dtype=np.int32)
            compact(dev_G, dev_Gscan, self.dev_Gindicator, self.dev_waypoints, self.dev_n, block=(threadsPerBlock,1,1), grid=(nBlocksPerGrid,1), stream=self.stream1)    
...
class GMTmem(object):
    ...
    def run_step(self, iter_parameters, iter_limit=1000, debug=False):
        ...
        while True:
            ...
            neighborIndicator(self.dev_xindicator, dev_G, self.dev_unexplored, self.dev_neighbors, self.dev_num_neighbors, self.neighbors_index, dev_gSize, block=(threadsPerBlock,1,1), grid=(gBlocksPerGrid,1), stream=self.stream1)
            ...
            #dev_x = cuda.zeros(xSize, dtype=np.int32)
            compact(dev_x, dev_xscan, self.dev_xindicator, self.dev_waypoints, self.dev_n, block=(threadsPerBlock,1,1), grid=(nBlocksPerGrid,1), stream=self.stream1)
